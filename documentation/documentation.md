...
- Deploys `privkey + fullchain + ca` to `$SC_DATASTORE_DIR/cert/<domain>.pem` and, when `/srv/<domain>/rootfs/etc/pki/tls/` exists, to the container's `private/localhost.key`, `certs/localhost.crt`, `certs/<domain>.pem` and `certs/localhost.pem`.
- The appended `/etc/letsencrypt/ca.pem` is the vendored `letsencrypt-ca.pem` — DST Root CA X3, **expired 2021-09-30** — so every deployed bundle ships an expired root; certbot's own `fullchain.pem` already carries the correct ISRG chain.
...
- Creates the `acme` system user (uid 528) and the `/var/acme` webroot, installs the vendored CA as `/etc/letsencrypt/ca.pem`, then generates, enables and starts `acme-server.service`.
