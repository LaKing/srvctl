...
**Issuance (primary, `regenerate` only):** `letsencrypt -c /etc/letsencrypt/srvctl-dns01.ini certonly --manual --preferred-challenges dns --manual-auth-hook "<hook> auth" --manual-cleanup-hook "<hook> cleanup" --cert-name srvctl-wildcard-<name> --keep-until-expiring -d <name> -d '*.<name>'` — a dedicated lineage, so the legacy http-01 lineages are never touched. Renewal at a third of the lifetime, at most `SC_ACME_MAX_ISSUE_PER_RUN` (10, `0` issues nothing) per run, per-name backoff 1 h → 24 h, `SC_ACME_DNS01_ONLY` (space- or comma-separated names) restricts issuance and renewal to the listed names for a controlled rollout — everything else stays `pending`, which holds every host's state — and `SC_LETSENCRYPT_STAGING=true` adds `--test-cert` (**never on a production primary**: nothing checks the issuer, so a test certificate would be distributed and would shadow the per-domain certificates in HAProxy). These knobs are plain `SC_…=` lines in `/etc/srvctl/*.conf`; `regenerate_letsencrypt` exports them to `letsencrypt.js` (the configs are sourced, not exported). The `-c` file (written by the run: email, `text = True`, no authenticator) replaces `cli.ini`, whose `authenticator = webroot` certbot refuses to combine with `--manual` ("Too many flags setting configurators"). Bundles and `index.json` are published in `/var/srvctl3/acme/bundles/` (0600) and deployed explicitly on the primary from `live/srvctl-wildcard-<name>/`.
