...
##
##   modules/ssh/libs/userlib.sh — disabled legacy user-key setup.
##
##   Sourced by load_libs while the ssh module is enabled, but the
##   top-level 'return' below skips the whole file, so nothing here is
##   ever defined. create_user_ssh was replaced by a nodejs
##   implementation in the usersonhost module and is kept only as a
##   reference — candidate to drop in v4.
##
...
function create_user_ssh() { ## user ## reseller
    ...
    user="$1"
    ## NOTE: reseller was presumably meant to be "$2" — dead code, kept as-is.
    reseller="$1"
